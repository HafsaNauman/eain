...
#  LLM CLIENT SETUP
# ────────────────────────────────────────────────────────────────
# ── Initialise OpenAI client once (lazy) ─────────────────────────
_openai_client = None
...
def _get_openai_client():
    ...
    if _openai_client is None:
        ...
        if not api_key:
            raise RuntimeError(
                "OPENAI_API_KEY not found. Put it in recommender_system/.env "
...
if len(df_labels) == 0:
    print("\n❌ No labels were generated — skipping consistency analysis.")
    print("   Check your API key and provider settings.")
    conf_mean = conf_high = conf_low = 0.0
    pref_A = pref_B = 0.0
    balanced = False
    coherence_rate = 0.0
    reason_quality = 0.0
...
stats = {
    "total_labels":    len(all_labels),
    "errors":          errors,
    "success_rate":    round(float(len(all_labels)/max(TARGET,1)), 4),
    "confidence_mean": round(float(conf_mean), 4),
    "high_conf_rate":  round(float(conf_high), 4),
    "low_conf_rate":   round(float(conf_low), 4),
    "preferred_A_pct": round(float(pref_A), 4),
    "preferred_B_pct": round(float(pref_B), 4),
    "balanced_split":  bool(balanced),
    "category_coherence": round(float(coherence_rate), 4),
    "reason_quality":  round(float(reason_quality), 4),
