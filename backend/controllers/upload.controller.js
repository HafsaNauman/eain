...
export const uploadProductImage = async (req, res) => {
  try {
    ...
    const { error } = await supabase.storage
      .from('product-images') // Different bucket for products
      .upload(fileName, fileBuffer, {
        contentType: file.mimetype,
      });
    ...
    if (error) {
      console.error('Supabase Upload Error:', error);
      return errorResponse(res, 500, 'Failed to upload product image', error.message);
    }
    ...
    return successResponse(res, 200, 'Product image uploaded successfully', {
      image_url: data.publicUrl,
    });
  } catch (error) {
