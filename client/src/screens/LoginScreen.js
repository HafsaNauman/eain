...
//         console.log('🔑 About to save - accessToken:', accessToken);
...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      // ❌ DISABLED Hardcoded admin bypass because 'dummy-access-token' 
      // will be rejected by your real backend with a 401 Unauthorized error!
      // You MUST log in normally to get a REAL JWT token from the backend.
      /*
      if (
        (fullPhoneNumber === '+923188108179' || fullPhoneNumber === '+9203188108179') &&
        password === 'hello123'
      ) {
        const dummyUser = {
          user_id: 'admin_123',
      ...
      */
      ...
      if (result.success) {
        // result.data is response.data from axios (the raw API body)
        ...
        if (!accessToken) {
          setError('Login failed: no token received. Check console for response shape.');
          setLoading(false);
          return;
        }
        ...
        if (user.role === 'admin') {
          ...
          navigation.reset({ index: 0, routes: [{ name: 'ServiceDashboard' }] });
