...
const ProfileScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const checkVendorStatus = async () => {
      if (isAuthenticated && user?.role === 'vendor') {
        setCheckingVendor(true);
        try {
          console.log('🔍 [ProfileScreen useEffect] Checking vendor status for user:', user.user_id);
          const profileCheck = await getVendorProfile();
          
          console.log('✅ [ProfileScreen useEffect] Profile check response:', JSON.stringify(profileCheck, null, 2));

          if (profileCheck?.success && profileCheck.data) {
            // ✅ Vendor with business → go to dashboard
            console.log('✅ [ProfileScreen useEffect] Vendor has business profile, redirecting to dashboard');
            navigation.reset({
              index: 0,
              routes: [{
                name: 'VendorDashboard',
            ...
            // ✅ Vendor without business → go to registration
            console.log('⚠️ [ProfileScreen useEffect] No business profile found, redirecting to registration');
            navigation.reset({
              index: 0,
              routes: [{
                name: 'BusinessRegistration',
          ...
          console.error('❌ [ProfileScreen useEffect] ERROR in checkVendorStatus:');
          ...
          // On error, assume needs registration
          console.log('⚠️ [ProfileScreen useEffect] Due to error, redirecting to business registration');
          navigation.reset({
            index: 0,
            routes: [{
              name: 'BusinessRegistration',
  ...
  const handleVendorDashboard = async () => {
    try {
      console.log('🔍 [handleVendorDashboard] Checking vendor profile for user:', user.user_id);
      const profileCheck = await getVendorProfile();
      
      console.log('✅ [handleVendorDashboard] Profile check response:', JSON.stringify(profileCheck, null, 2));
      
      if (profileCheck.success && profileCheck.data) {
        console.log('✅ [handleVendorDashboard] Navigating to VendorDashboard');
        navigation.navigate('VendorDashboard', {
          userId: user.user_id,
          vendorProfile: profileCheck.data.data?.profile || profileCheck.data.profile
        });
      } else {
        console.log('⚠️ [handleVendorDashboard] No profile found, navigating to BusinessRegistration');
        navigation.navigate('BusinessRegistration', {
          userId: user.user_id,
          userRole: 'vendor'
        });
      ...
      // 🔴 Detailed error logging
      console.error('❌ [handleVendorDashboard] ERROR navigating to dashboard:');
      console.error('Error type:', error.constructor.name);
      console.error('Error message:', error.message);
      ...
      console.log('⚠️ [handleVendorDashboard] Due to error, navigating to BusinessRegistration');
      navigation.navigate('BusinessRegistration', {
        userId: user.user_id,
        userRole: 'vendor'
      });
