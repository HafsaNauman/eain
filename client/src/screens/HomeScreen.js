...
// import { getForYouFeed, voiceRerank, logEvent } from '../api/recommendService';
...
//   const [feedSource, setFeedSource] = useState('catalog'); // 'catalog' | 'recommender' | 'voice'
...
//   // ── on mount: get user_id and load recommender feed ───────────────────────
//   useEffect(() => {
//     (async () => {
//       const uid = await AsyncStorage.getItem('user_id');
//       setCurrentUserId(uid);
//       if (uid) {
//         await fetchRecommenderFeed(uid);
//       } else {
//         fetchListings();
//       }
//     })();
...
//   // ── recommender feed ──────────────────────────────────────────────────────
//   const fetchRecommenderFeed = async (userId) => {
//     setLoading(true);
//     setError('');
//     try {
//       const rec = await getForYouFeed(userId);
//       if (rec.success && rec.data?.results?.length > 0) {
//         const mapped = rec.data.results.map(r => ({
...
//           price: r.price,
//           currency: 'PKR',
//           media: { images: [`https://via.placeholder.com/150?text=${encodeURIComponent(r.title)}`] },
//           _rec_score: r.score,
...
//         console.log(`✅ Recommender feed: ${mapped.length} items (${rec.data.method})`);
//         return;
//       }
//     } catch (e) {
//       console.warn('Recommender unavailable, falling back to catalog');
//     }
//     // fallback to catalog if recommender fails
//     fetchListings();
//   };

//   // ── catalog fetch (existing logic) ───────────────────────────────────────
//   const fetchListings = async (isRefresh = false) => {
//     try {
//       if (isRefresh) {
//         setRefreshing(true);
...
//         setFeedLabel('');
//         console.log(`✅ Catalog: ${result.data.listings.length} products`);
//       } else {
//         setError(result.error);
//       }
//     } catch (err) {
...
//   const handleRefresh = () => {
//     if (currentUserId && feedSource !== 'catalog') {
...
//     setSelectedStockFilter('all');
//     setShowFilters(false);
//     // restore recommender feed after clearing
//     if (currentUserId) fetchRecommenderFeed(currentUserId);
//   };

//   // ── voice search with recommender rerank ─────────────────────────────────
//   const handleVoiceSearch = async () => {
//     if (isVoiceRecording) {
//       setIsVoiceRecording(false);
//       setIsVoiceProcessing(true);
...
//             } else {
//               // fallback: regular catalog search
//               setSearchQuery(normalized.trim());
//             }
//             console.log(`🎤 Voice: "${displayText}" → normalized: "${normalized}"`);
//           } else {
//             Alert.alert('No speech detected', 'Please try again and speak clearly.');
//           }
//         } else {
...
//   // ── navigate to product detail + log click event ──────────────────────────
//   const navigateToProductDetail = (listingId) => {
//     if (currentUserId) logEvent(currentUserId, listingId, 'click');
//     navigation.navigate('CustomerProduct', { listingId });
//   };
...
//             onChangeText={(text) => {
//               setSearchQuery(text);
//               if (!text.trim() && currentUserId) fetchRecommenderFeed(currentUserId);
//             }}
//             returnKeyType="search"
//           />
//           {searchQuery.length > 0 && (
...
//         <Text style={styles.sectionTitle}>Services</Text>
//         <View style={{ marginBottom: 20, gap: 10 }}>
//           <TouchableOpacity style={styles.serviceCta} onPress={() => navigation.navigate('ServiceBrowse')} activeOpacity={0.85}>
//             <View style={{ flex: 1 }}>
//               <Text style={styles.serviceCtaTitle}>Book a Service</Text>
//               <Text style={styles.serviceCtaSub}>Makeup · Photography · Mehndi · Catering & more</Text>
//             </View>
...
//             </View>
//           </TouchableOpacity>
//           <TouchableOpacity style={styles.myBookingsBtn} onPress={() => navigation.navigate('MyBookings')} activeOpacity={0.85}>
//             <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
//               <Ionicons name="calendar-outline" size={20} color="#036c5f" />
//               <Text style={styles.myBookingsText}>My Bookings</Text>
//             </View>
...
//         {!loading && (
//           <>
//             {/* Feed label: "For You · AI" or "🎤 voice query" or empty */}
//             <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
//               <Text style={styles.sectionTitle}>
//                 {feedSource === 'recommender' ? '✨ For You' : feedSource === 'voice' ? '🎤 Voice Results' : `${t('homeScreen.products')} (${products.length})`}
//               </Text>
//               {feedLabel ? (
//                 <Text style={styles.feedBadge}>{feedLabel}</Text>
//               ) : null}
//               {(feedSource === 'recommender' || feedSource === 'voice') && (
//                 <TouchableOpacity onPress={() => fetchListings()} style={styles.seeAllBtn}>
...
//                     />
//                   )}
//                   {/* AI score badge for recommender feed */}
//                   {product._rec_score && (
//                     <View style={styles.recScoreBadge}>
//                       <Text style={styles.recScoreText}>✨ {(product._rec_score * 100).toFixed(0)}%</Text>
//                     </View>
//                   )}
//                   <Text style={styles.productName} numberOfLines={2}>
//                     {isUrdu && product.title_ur ? product.title_ur : product.title_en}
...
//           ))}
//         </ScrollView>
//         {wishlist.length === 0 && <Text style={styles.noFavorites}>{t('homeScreen.noFavorites')}</Text>}
...
//   categorySelected: { backgroundColor: '#036c5f' },
//   sectionTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 12, marginTop: 8, color: '#036c5f' },
//   feedBadge: { fontSize: 11, color: '#036c5f', backgroundColor: '#e0f7fa', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
//   seeAllBtn: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, backgroundColor: '#f0f0f0' },
...
//   productPrice: { color: '#036c5f', fontWeight: 'bold', marginBottom: 8, fontSize: 14 },
//   stockBadge: { position: 'absolute', top: 8, right: 8, width: 60 },
//   recScoreBadge: { position: 'absolute', top: 8, left: 8, backgroundColor: 'rgba(3,108,95,0.85)', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 2 },
...
const normalizeProduct = (item = {}) => {
  const images =
    item?.media?.images ||
    (Array.isArray(item?.media) ? item.media.map((m) => m?.imageurl).filter(Boolean) : []) ||
    (item?.media?.imageurl ? [item.media.imageurl] : []) ||
        ...
        {!loading && (
          <>
            ...
            >
              ...
              {feedLabel ? <Text style={styles.feedBadge}>{feedLabel}</Text> : null}
                  ...
                  {product._rec_score ? (
                    <View style={styles.recScoreBadge}>
                      <Text style={styles.recScoreText}>
                        ✨ {(product._rec_score * 100).toFixed(0)}%
                      </Text>
                    </View>
                  ) : null}
